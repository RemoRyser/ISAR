/*
    Code for the article 'Landscape configurations and trophic structures interactively drive species-area relationships in meta-food-webs'
...
Johanna Häussler, Remo Ryser, Markus Stark
...
double tend=50000;//5e5;           // time at which simulation runs stop
double teval=10000;//2e4;           // length of time interval from which to determine mean(B) and CV(B)
double Delta_t=1;//0.01;         // step size in ODE integration (internal step size can be less or larger)

const double eps_rel= 1e-6;  // relative error tolerance for ODE solver
const double eps_abs = 1e-6; // absolute error tolerance for ODE solver
const double EXTINCT = 1;  // extinction threshold for biomass densities
...
double mu_S=50;               // mean nutrient supply concentration
...
int WRITEPARAMSTOFILE = 1;    // if true, the parameter array is written to a file
...
int KILLSWITCH = 1;           // 0: initialising all species on all patches; 1: kill some species on patches during initialisation
...
double emigr_a_S_b=0.1;                         // max. emigration rate#0.1
double emigr_b_S_b=0;                         // shape of curve for emigration#0
...
double emigr_a_S_c=0.1;                         // max. emigration rate#0.1
double emigr_b_S_c=10;                         // shape of curve for emigration#10
...
const double MIN_MIGRATION = Delta_t;         // minimum migration threshold
...
//char* landscape_s = getenv("LANDSCAPE"); 		// read in landscape size from bash
...
double *web_calc(gsl_rng *r)
{
    ...
    gsl_vector *Area      = gsl_vector_calloc(Z);
    gsl_vector *AreaExtinct = gsl_vector_calloc(D+DN);
    ...
    int paramslength = 2*S*S+7*S+S_b*N+N+S_b+2+S*Z*Z+2*Z+7+D+DN+D+DN;
    ...
    Generate_RGG_structure(r,mass,Loc,SW,con,params,RGG_params,DmaxPlant,Area, AreaExtinct);
    printf("4 RGG generated\n");
    

    set_spatial_parameters(r,SW,Loc,params);                // set random spatial parameters for the simulation run
    ...
    solve_ode(B,meanB,meanB_tot,VarCoeffB,VarCoeffB_tot,params,CovM,Biomass_Temp,net_growth_Temp,AreaExtinct);   // apply solver for integration of the ODE
    printf("7 ODE solved\n");

    output(SW,mass,con,iniB,meanB,meanB_tot,VarCoeffB,VarCoeffB_tot,params,paramslength,Biomass_Temp,RGG_params,net_growth_Temp, Area);
...
//  *********** Free memories ***********
    ...
    gsl_vector_free(con);
    gsl_vector_free(Loc);
    gsl_vector_free(Area);
    gsl_vector_free(AreaExtinct);
    free(params);
    free(B);
    free(iniB);
    free(meanB);
...
static void set_parameters(gsl_rng *r, gsl_matrix *Ap, gsl_vector *mass, double params[], gsl_matrix *Up)
{
    ...
    {
        if(i >= S_b)
        // the following lines are only for non-basal species
        {
...
static void set_spatial_parameters(gsl_rng *r, gsl_matrix *SW, gsl_vector *Loc, double params[])
{
...
//***************************************************************************************
static void solve_ode(double B[], double meanB[], double meanB_tot[], double CV[], double CV_tot[], double params[], gsl_matrix *CovM, double Biomass_Temp[], double net_growth_Temp[], gsl_vector *AreaExtinct)
{
    int i,j,m,flag,ERR_FLAG;
    double t = 0;
    double tempo,tempBm;// starting time
    double *pparams=(double *)params;          									// pointer to first element of params...
    ...
    gsl_vector_view net_growthrate = gsl_vector_view_array(pparams+2*S*S+6*S+S_b*N+N+S_b+2+S+S*Z*Z+S*Z+N*Z+2*Z+7,D); // net growth rate
    ...
    double tout = 1;//0.01;                                        // initial step size
...
// ********* integrate ODE to reach an attractor; optional: output of time series ********
    ...
    while(t < tend)
    {
        ...
        for (i=0; i<D+DN; i++)
        {
            tempo=gsl_vector_get(AreaExtinct,i);
            //printf("%g, %g\n", tempo, B[i]);
            if (B[i] < tempo && B[i] != 0)                     		// determine whether a species just fell below the extinction threshold
            {
                EXT_FLAG = 1;
            }
        ...
        if (EXT_FLAG)                                          			// if an extinction occured:
        {
            ...
            Extinction(B,D+DN,AreaExtinct);                                     	// set the biomass of the species to 0...
            tout = t+Delta_t;                                       		// set the next output time to a reasonable value...
...
//***************************************************************************************
static void Extinction(double B[], int Num, gsl_vector *AreaExtinct)
{
    int i;
    for (i=0; i<Num; i++)
    {
        ...
        if (B[i] < gsl_vector_get(AreaExtinct,i))
        {
            B[i] = 0;
...
static void dynamics(double B[], double Bdot[], void *params)
{
    ...
    double temp;// pointer to first element of params
    ...
    gsl_vector_view AE_vec=gsl_vector_view_array(pparams+2*S*S+6*S+S_b*N+N+S_b+2+S+S*Z*Z+2*Z+7+D+DN,D+DN);
    gsl_vector *AEvec=&AE_vec.vector;
...
// intercept too small migrations
    for(i=0; i<D; i++)
    {
        ...
        temp=gsl_vector_get(AEvec,i) / MIN_MIGRATION;
        if(gsl_vector_get(Imvec,i)<temp && gsl_vector_get(Imvec,i) != 0) {
            gsl_vector_set(Imvec,i,0);
        }
        
        //printf("%g\n", gsl_vector_get(Imvec,i));
...
//***************************************************************************************
//	recursive function to efficiently calculate the Covariance Matrix of a species/group across the patch network with changing lower boundary (lB)
//***************************************************************************************
static void calc_CovM(gsl_vector *Bvec_i, gsl_vector *Mvec_i, gsl_matrix *CovM_i, int j, int lB, int uB)
{
    int k;
    double tempV; // can be Var, Covar

    for(k=lB; k<uB; k++)
    {
        tempV = gsl_matrix_get(CovM_i, j, k);
        tempV += (gsl_vector_get(Bvec_i, j)-gsl_vector_get(Mvec_i, j))*(gsl_vector_get(Bvec_i, k)-gsl_vector_get(Mvec_i, k))*Delta_t/teval;			// Calculate coavriance matrix
        //printf("%d,%d \n",j, i);
        //CV[i] += (B[i]-meanB[i])*(B[i]-meanB[i]) * Delta_t/teval; // Varianz, nicht CV
        //gsl_matrix_set(CovM, j, i, j*i);
        //gsl_matrix_set(CovM, i, j, j*i);
        gsl_matrix_set(CovM_i, j, k, tempV);
        gsl_matrix_set(CovM_i, k, j, tempV);
    }

    if(j<uB && lB<uB)
        calc_CovM(Bvec_i, Mvec_i, CovM_i, j+1, lB+1 ,uB);
...
//***************************************************************************************
static void Generate_RGG_structure(gsl_rng *ran, gsl_vector *mass, gsl_vector *Loc, gsl_matrix *SW,  gsl_vector *con, double params[], double RGG_params[], gsl_vector *DmaxPlant, gsl_vector *Area, gsl_vector *AreaExtinct)
{

    int i,m,n,j,k;
    double X_1,X_2,Y_1,Y_2,DistX,DistY, d_mn, d_spec, sum,temp5, D_max_scale, D_max_max, tempArea;
    //landscape_size = atof(landscape_s);           // read in landscape size from bash
    //landscape_size = 1;
    ...
    D_0 = 1;  // to scale D_0 to landscape size
...
// printf("%g\t%g\t%g",eps,D_0,theta);
    ...
    gsl_matrix *Area_W = gsl_matrix_calloc(Z,Z);            // Area ration for area weightet success
    ...
    for (i=0; i<S_b; i++)
    {
        ...
        gsl_vector_set(D_Max, i, D_0*temp5*0);        // Calculate max dispersal distance for basal species

    }
    }
    for (i=S_b; i<S; i++)
        gsl_vector_set(D_Max, i, D_0*D_max_scale*pow(gsl_vector_get(mass,i)*0, eps));  // Calculate max dispersal distance for consumer species

//	 printf("1 D_Max\n");
//	 show_vector(D_Max, S);

    if(INPUT_LANDSCAPE)
        getInputCoords(Loc, Area);
        ...
        if(gsl_vector_get(Loc, 0) == 0 && gsl_vector_get(Loc, 1) == 0 && gsl_vector_get(Loc, 3) == 0)    // it's probably empty
        {
            ...
            for (m=0; m<Z; m++)
            {
                ...
                gsl_vector_set(Area,m,gsl_rng_uniform(ran));
            }
        }
    }
    ...
    for (i=0; i<Z; i++) {
        for (j=0; j<Z; j++) {
            ...
            tempArea = gsl_vector_get(Area,j)/gsl_vector_get(Area,i);
            gsl_matrix_set(Area_W,i,j,tempArea);
...
//			printf("4 \n");
    ...
    for(i=0; i<S; i++)
    {
        ...
        gsl_matrix_mul_elements(SW_i, Area_W);
    ...
    for (i=0; i<S; i++) {
        for (j=0; j<Z; j++) {
            ...
            gsl_vector_set(AreaExtinct,j*S+i,(EXTINCT / gsl_vector_get(Area,j) * gsl_vector_get(mass,i)));
        }
    }
    
    for (i=D;i<(D+DN);i++) {
        gsl_vector_set(AreaExtinct,i,1e-20);
    }
    
    for (i=0;i<D+DN;i++) {
        *(params+2*S*S+6*S+S_b*N+N+S_b+2+S+S*Z*Z+2*Z+7+D+DN+i)=gsl_vector_get(AreaExtinct,i);
    }
    //show_vector(Area,Z);
    //show_vector(AreaExtinct,S*Z);
...
//***************************************************************************************
static void output(gsl_matrix *SW, gsl_vector *mass,  gsl_vector *con, double iniB[], double meanB[], double meanB_tot[], double CV[], double CV_tot[], double params[], int paramslength, double Biomass_Temp[], double RGG_params[], double net_growth_Temp[], gsl_vector *Area)
{
    int i,j,k,m;
    ...
    fprintf(file3,"%d,%d,%d,%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g\n",S,S_b,S_c,Z,seed,
            emigr_a_S_b,emigr_b_S_b,emigr_a_S_c,emigr_b_S_c,D_0,theta,eps,
            RGG_params[0],RGG_params[8],RGG_params[1],RGG_params[9],RGG_params[2],RGG_params[3],RGG_params[4],RGG_params[5],RGG_params[6],RGG_params[7],g);

    // mass.out
    fprintf(file2, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n","patch","species","body.mass", "init.biomass", "mean.biomass","biomass.variance","tot.mean.biomass","tot.biomass.variance","if.basal.spp","Biomassess_tend-20.000","Biomassess_tend-10.000","Biomassess_tend","spatial.connectance","Mean.net.growth1","Mean.net.growth2","net.growth.sd","area");  //local.alpha.var = CoefficientOfVariation
    ...
    for(j=0; j<Z; j++)
    {
        ...
        for(i=0; i<S; i++)
            fprintf(file2,"%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g\n", j, i, gsl_vector_get(mass,i), iniB[j*S+i],meanB[j*S+i],CV[j*S+i],meanB_tot[i],CV_tot[i],gsl_vector_get(Pvec,i),Biomass_Temp[i+j*S],Biomass_Temp[D+DN+i+j*S],Biomass_Temp[2*(D+DN)+i+j*S],gsl_vector_get(con,i),net_growth_Temp[j*S+i],net_growth_Temp[D+j*S+i],net_growth_Temp[2*D+j*S+i],gsl_vector_get(Area,j));

        //nurients -- don't forget to include output !!!
        for(m=(D+j*N); m<(D+j*N+1); m++)
            fprintf(file2,"%d,%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",j, 1000, 0, iniB[m],meanB[m],CV[m],meanB_tot[m],CV_tot[m],2,0,0,0,0,0,0,0,0);
        for(m=(D+j*N+1); m<(D+j*N+N); m++)
            fprintf(file2,"%d,%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",j, 2000, 0, iniB[m],meanB[m],CV[m],meanB_tot[m],CV_tot[m],2,0,0,0,0,0,0,0,0);
    }
...
static void Write_landscape_to_file(double params[])
{
    ...
    for (int i=0; i<Z; i++)
        fprintf(file1, "%g, %g\n", params[2*S*S+7*S+N*S_b+N+S_b+2+S*Z*Z+S*Z+N*Z+0+2*i], params[2*S*S+7*S+N*S_b+N+S_b+2+S*Z*Z+S*Z+N*Z+1+2*i]);
    fclose(file1);
...
static void getInputMass(gsl_vector *mass, gsl_matrix *Up,gsl_vector *DmaxPlant)
{
    ...
    std::string col2, col4, col6, col8; // needed for comma-seperator
    double col1,col3, col5, col7, col9;
    
    while(getline(input,line))
    {
        std::istringstream iss(line);
        iss >> col1 >> col2 >> col3 >> col4 >> col5 >> col6 >> col7 >> col8 >> col9;
        masses.push_back(col1);
        dispersaldist.push_back(col5);
        NutUptake1.push_back(col7);
        NutUptake2.push_back(col9);
    }
    ...
    for(unsigned i=0; i<S_b;i++)
    {
        ...
        gsl_matrix_set(Up,i,1,NutUptake2[i+1]);
...
static void getInputLandscape()
{
    ...
    if(!input.is_open())
    {
        std::cerr << "There was a problem opening the RGG input file3\n";
        exit(1);
    }
...
static void getInputCoords(gsl_vector *coords_in_v, gsl_vector *area_in_v)
{
    ...
    std::string col2; // needed for comma-seperator
    std::string col4; // needed for comma-seperator
    std::string col6; // needed for comma-seperator
    double col1,col3,col5;
    int col7;
    while(getline(input,line))
    {
    ...
    for(int i=0; i<Z; i++)
    {
        gsl_vector_set(coords_in_v,0+i*2,coords_in_X[i+1]);
        gsl_vector_set(coords_in_v,1+i*2,coords_in_Y[i+1]);
        gsl_vector_set(area_in_v,i,area_in[i+1]);
